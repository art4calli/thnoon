...
export default function AboutSection({ cards, header }: AboutSectionProps) {
  ...
  // Set the dynamic bio fields with proper defaults
  const sectionBadge = header?.badge || "إرث حضاري إسلامي خالد";
  const sectionTitle = header?.title || "عن مؤسسة يوسف ذنون للخط العربي";
  const sectionDescription = header?.description || "تأسست المؤسسة لتكون مناراً إسلامياً وثقافياً يحمل اسم الراحل الكبير الأستاذ يوسف ذنون، عميد ومؤرخ الخط العربي، لنشر هذا الفن الشريف ورعاية أجيال الخطاطين في شتى أنحاء العالم الإسلامي.";
  
  const bioName = "الأستاذ يوسف ذنون";
  const bioSubtitle = "عميد ومؤرخ الخط العربي والآثار الإسلامية (١٩٣٢ - ٢٠٢٠)";
  const bioTitle = "سيرة عميد الخطاطين ونشأة المؤسسة";
  const bioDesc1 = "ولد الخطاط والمؤرخ العراقي الأستاذ يوسف ذنون في مدينة الموصل الحدباء عام ١٩٣٢. كرس حياته لخدمة كتابة المصحف الشريف والبحث في أصول الحرف العربي وعلم الآثار والنقوش الإسلامية. كاتب بارز ومحاضر دولي أجاز عشرات الخطاطين الذين أصبحوا بدورهم قادة فن الخط في مختلف الأقطار.";
  const bioDesc2 = "تحولت المؤسسة بجهود تلامذته والمحبين لفنه إلى منبر ثقافي يضم مكتبته النادرة، مخطوطاته الإبداعية، ودراساته التأصيلية، لتقديم برامج تعليمية مميزة تعتمد على إجازته ومدرسته الفريدة في كتابة خطوط الثلث والنسخ والديواني والكوفي.";
  const bioImage = "https://images.unsplash.com/photo-1578301978693-85fa9c0320b9?auto=format&fit=crop&q=80&w=600";

  const stat1Value = "٥٠+";
  const stat1Label = "سنة من العطاء العلمي والفني";
  const stat2Value = "١٠٠+";
  const stat2Label = "دراسة وبحث تخصصي في الآثار";
  const stat3Value = "٥٠٠+";
  const stat3Label = "تلميذ وخطاط مجاز حول العالم";
  ...
  return (
    <section className="py-20 px-4 max-w-7xl mx-auto animate-fade-in">
      ...
      {hasCustomCards ? (
            ...
            >
              <div>
                {card.media && card.media.length > 0 ? (
                  ...
                  <div className="aspect-video bg-slate-900/40 rounded-xl flex items-center justify-center border border-slate-800 mb-4">
        ...
        /* Fallback structure when no cards exist in the Google Sheet */
        <div className="space-y-20">
          {/* Main Biography Card */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center bg-slate-950/40 border border-slate-800/60 rounded-3xl p-6 sm:p-10 shadow-2xl">
